...
namespace Microsoft.AspNetCore.OData.Formatter
{
    ...
    public class ODataOutputFormatter : TextOutputFormatter
    {
        ...
        private void WriteResponseBody(OutputFormatterWriteContext context)
        {
            ...
            IODataResponseMessage responseMessage = ODataMessageWrapperHelper.Create(response.Body, response.Headers);
            ...
            MediaTypeHeaderValue contentType = MediaTypeHeaderValue.Parse(context.ContentType.Value);
            IEnumerable<KeyValuePair<string, string>> parameters =
                contentType.Parameters.Select(val => new KeyValuePair<string, string>(val.Name, val.Value));
            ODataMetadataLevel metadataLevel = ODataMediaTypes.GetMetadataLevel(contentType.MediaType, parameters);
        ...
        private ODataSerializer GetSerializer(Type type, object value, OutputFormatterWriteContext context)
        {
            ...
            if (edmObject != null)
            {
                ...
                serializer = serviceProvider.GetEdmTypeSerializer(edmType);
                if (serializer == null)
                {
                    string message = Error.Format(SRResources.TypeCannotBeSerialized, ToTraceStringExtensionMethods.ToTraceString(edmType), typeof(ODataOutputFormatter).Name);
            ...
            else
            {
                ...
                serializer = serviceProvider.GetODataPayloadSerializer(type, new WebApiRequestMessage(context.HttpContext.Request));
                if (serializer == null)
                {
        ...
        private ODataPayloadKind? GetClrObjectResponsePayloadKind(Type type, OutputFormatterWriteContext context)
        {
            ...
            ODataSerializer serializer = provider.GetODataPayloadSerializer(type, new WebApiRequestMessage(context.HttpContext.Request));
